...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (!email) {
    ...
    return Response.json({ success: true, remaining: Infinity, limit: Infinity });
  }
  ...
  if (!supabaseUrl || !supabaseKey) {
    ...
    return Response.json({ success: true, remaining: dailyLimit, limit: dailyLimit });
  }
  ...
  try {
    ...
    if (existing) {
      ...
      if (updateError) {
        ...
        return Response.json({ success: true, remaining: dailyLimit - currentCount, limit: dailyLimit });
      }
    } else {
