        ...
        {msg.relatedLinks && msg.relatedLinks.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-0.5">
            {msg.relatedLinks.map((link) => (
              <a key={link.href + link.label} href={link.href} className="inline-flex items-center gap-1 rounded-lg border border-indigo-500/30 bg-indigo-500/10 px-2 py-1 text-[11px] font-medium text-indigo-300 transition-colors hover:bg-indigo-500/20 hover:text-indigo-200">
    ...
    <motion.div initial={{ opacity: 0, y: 8, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.2, ease: "easeOut" }} className={`flex gap-2 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      ...
      <div className={`flex max-w-[85%] flex-col gap-1.5 ${isUser ? "items-end" : "items-start"}`}>
        ...
        {!isUser && msg.relatedLinks && msg.relatedLinks.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-0.5">
            {msg.relatedLinks.map((link) => <a key={link.href + link.label} href={link.href} className="inline-flex items-center gap-1 rounded-lg border border-indigo-500/30 bg-indigo-500/10 px-2 py-1 text-[11px] font-medium text-indigo-300 transition-colors hover:bg-indigo-500/20 hover:text-indigo-200">{LINK_ICON_MAP[link.icon]}{link.label}</a>)}
          </div>
        )}
...
function UpgradeNudge() {
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="mx-2 mb-2 flex flex-col items-center gap-2 rounded-xl border border-saffron/30 bg-gradient-to-br from-saffron/10 via-gold/5 to-transparent p-3 text-center">
      ...
      <a href="/app/pricing" className="inline-flex items-center gap-1.5 rounded-lg bg-saffron px-3 py-1.5 text-xs font-bold text-background transition-opacity hover:opacity-90"><Crown className="size-3" />Upgrade to Pro</a>
    </motion.div>
  );
}
            ...
            {hitLimit ? <UpgradeNudge /> : (
              <div className="shrink-0 border-t border-border/50 bg-background/80 px-3 py-2.5 pb-safe">
                {!isActivePro && mitraLimit.remaining > 0 && mitraLimit.limit !== Infinity && mitraLimit.remaining < mitraLimit.limit && (
                  <p className="mb-1.5 text-center text-[10px] text-muted-foreground">{mitraLimit.remaining} free message{mitraLimit.remaining !== 1 ? "s" : ""} remaining {"\u2022 "}<a href="/app/pricing" className="text-saffron hover:underline">Upgrade for unlimited</a></p>
                )}
